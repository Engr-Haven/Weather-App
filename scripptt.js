// Select a random city when the page loads
window.onload = function () {
    let randomCities = ["New York", "London", "Abuja", "Tokyo", "Paris", "Dubai", "Sydney", "Toronto", "Berlin", "Mumbai", "Cairo", "Lagos"];
    let randomCity = randomCities[Math.floor(Math.random() * randomCities.length)];
    
    // Use the same logic as the submit event to fetch and display weather
    fetchWeather(randomCity);
};

// Your existing form event listener
...
inputField.addEventListener(`focus`, clearError)
function clearError(){
...
// Function to fetch weather (Same logic as your existing code)
